...
/* ===== Fix Slider Tick Values (0 and 100) ===== */
.stSlider div[data-baseweb="slider"] span {
    color: #ffffff !important;
    font-weight: 600 !important;
}

/* Force visibility of min/max numbers */
...
with tab4:
    ...
    monthly = df_filtered.resample("M",on="Order Date").agg(
    Revenue=("Sales","sum"),
    Profit=("Gross Profit","sum")
    ).reset_index()
...
# -------------------------------------------------
    ...
    margin_trend = df_filtered.resample("M", on="Order Date").agg(
        Margin=("Gross Margin %", "mean")
    ).reset_index()
